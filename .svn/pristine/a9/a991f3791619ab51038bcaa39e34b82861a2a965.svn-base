...
<script src="${rootUrl }js/plugins/jui/jquery-ui-1.9.2.min.js" type="text/javascript">
		...
		<div class="window">
			...
			<div class="mtab">
				<table width="100%">
					...
					<tr>
						<c:choose>
							<c:when test="${pageModel.faultStatus=='SOLVED' || pageModel.faultStatus=='RESOLVED'}">
								<th width="13%">预估确认时间:</th>
								<td colspan="5" width="20%"><fmt:formatDate
								value="${pageModel.confirmTime}" type="date"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
							</c:when>
						</c:choose>
					</tr>
