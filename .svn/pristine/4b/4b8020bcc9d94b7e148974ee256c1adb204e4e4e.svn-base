...
<tr>
	...
	<td width="7%"><fmt:formatDate value="${vo.planOpenTime.time}" type="date"
			pattern="yyyy-MM-dd" /></td>
	<td width="7%"><fmt:formatDate value="${vo.planEndTime.time}" type="date"
			pattern="yyyy-MM-dd" /></td>
	...
	<td width="10%"><fmt:formatDate value="${vo.updateTime.time}" type="date"
			pattern="yyyy-MM-dd HH:mm:ss" /></td>
	<td class="tdbtn" width="15%"><a
		...
		class="my_add{boxwidth:850,boxheight:500}">日志</a> <c:if
			...
			<a href="${rootUrl}tempcircuit/tesdb/view.do?tempcircuitid=${vo.id}"
				current class="my_view {boxwidth:850,boxheight:500}">电路明细</a>
		</c:if> <c:if test="${vo.circuitType=='TESVOICE'}">
			<a
				href="${rootUrl}tempcircuit/tesaudio/view.do?tempcircuitid=${vo.id}"
				class="my_view {boxwidth:850,boxheight:500}">电路明细</a>
		</c:if> <c:if test="${vo.circuitType=='KUDATA'|| vo.circuitType=='KUBROADCAST'}">
			<a href="${rootUrl}tempcircuit/ku/view.do?tempcircuitid=${vo.id}"
				class="my_view {boxwidth:850,boxheight:500}">电路明细</a>
		</c:if></td>
	<td class="tdbtn" width="10%"><c:if test="${!empty vo.engineerName}">
			<c:if test="${fn:indexOf(vo.engineerName,currentUser) > -1}">
				<a href="#" onclick="ackdelete('${vo.id}')">确认撤单</a>
			</c:if>
		</c:if> <c:if test="${empty vo.engineerName}">
			<a href="#" onclick="ackdelete('${vo.id}')">确认撤单</a>
		</c:if></td>
</tr>
