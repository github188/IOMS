		...
		$("#modForm").validate({
			rules : {			
				...
				circuitName : {
					...
					remote:'${rootUrl}basedata/checkTesService.dox?excludeId='+$("#id").val()
				},
...
<body>
	<form:form modelAttribute="pageModel" id="modForm" name="modForm"
		action="${rootUrl }basedata/tes/service/mod.do" method="post">
		<form:hidden path="id" />
		<form:hidden path="clientInfo" />
		<div class="window">
			<dl class="title">
				<dd>数据管理--TES业务--修改</dd>
				<dt>
			...
			<table style="margin: auto">
				<tr>
					...
					<td width="30%"><form:hidden path="circuitName" />${pageModel.circuitName}</td>
				</tr>
				<tr>
					<th width="20%"><font color="red">(*)</font>主叫端机箱号:</th>
					<td><form:input path="activeNodeCode"
							class="text ui-widget-content" cssStyle="width:100px" /></td>
				...
				<tr>
					...
					<%-- <td>
					<form:select path="activeSlot" cssStyle="width:100px">
							<form:option value="${pageModel.activeSlot}">${pageModel.activeSlot}</form:option>
						</form:select>
						
						<form:select path="activeSlot" cssStyle="width:135px">						
						<form:options items="${pageModel.activeSlots}" itemLabel="value" />
						</form:select>
					...
					<td><form:select path="activeSlot" cssStyle="width:105px">
							<form:options items="${pageModel.activeSlots}" />
						</form:select></td>


					<th width="20%"><font color="red">(*)</font>被叫端槽位号:</th>
					<%-- 	<td><form:select path="passiveSlot" cssStyle="width:100px">
							<form:option value="${pageModel.activeSlot}">${pageModel.passiveSlot}</form:option>
						</form:select>		
						 --%>
					<td><form:select path="passiveSlot" cssStyle="width:105px">
							<form:options items="${pageModel.passiveSlots}" />
						</form:select></td>
