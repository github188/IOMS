...
<tr>
	...
	<c:forEach items="${vo.psOtCheckContent }" var="pocc">
        ...
        <td>
            <c:choose>
                <c:when test="${pocc.underTlDlwrCheckResult eq 'INNORMAL'}">
                    <font>${pocc.underTlDlwrCheckResult.value}</font>
                </c:when>
                <c:otherwise>
                    ${pocc.underTlDlwrCheckResult.value}
                </c:otherwise>
            </c:choose>
        </td>
        <td>${pocc.underTlDlwrRemarks }</td>
        <!-- 一楼电话线路、数据线路配线线架 -->
        <td>
            <c:choose>
                <c:when test="${pocc.ffTlDlwrCheckResult eq 'INNORMAL'}">
                    <font>${pocc.ffTlDlwrCheckResult.value}</font>
                </c:when>
                <c:otherwise>
                    ${pocc.ffTlDlwrCheckResult.value}
                </c:otherwise>
            </c:choose>
        </td>
        <td>${pocc.ffTlDlwrRemarks }</td>
        <!-- 二楼电话线路、数据线路配线线架 -->
        <td>
            <c:choose>
