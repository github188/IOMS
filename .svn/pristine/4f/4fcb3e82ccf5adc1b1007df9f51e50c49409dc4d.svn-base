 ...
 */
@Controller
@RequestMapping("/duty/satellitedayauto/overseacdm/")
public class OverseaCdmController extends MyBaseController {
	...
	public String addJump(
			@ModelAttribute("pageModel") SatTxjdOverseaCdmModel pageModel,
			@RequestParam("optType") String optType, Model model) {
		// 检查结果枚举类
		model.addAttribute("optType", optType);
		// 跳转添加页面
		return "duty/satellitedayauto/txjdoverseacdm/add";
	}
	...
	public String delete(
		...
		try {
			// 删除
			satTxjdOverseaCDMService.delete(pageModel);
			// 返回
		...
		return "duty/satellitedayauto/txjdoverseacdm/result";
	}
	...
	public String updateJump(
		...
		// 检查结果枚举类
		Pagination pagn = satTxjdOverseaCDMService.queryPageByModel(queryModel);
		// 取出列表
		List<SatTxjdOverseaCdm> mains = (List<SatTxjdOverseaCdm>) pagn
				.getResult();
		// 非空验证
		...
		return "duty/satellitedayauto/txjdoverseacdm/update";
	}
