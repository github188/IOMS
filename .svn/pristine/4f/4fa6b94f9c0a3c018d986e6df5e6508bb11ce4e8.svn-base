 ...
 */
@Controller
@RequestMapping("/duty/statellite/kusys/")
public class KuSysController extends MyBaseController {
	@Autowired
	...
	public String addJump(@ModelAttribute("pageModel") KuSys queryModel,
		...
		return "duty/statellite/kusys/add";
	}
	...
	public String add(@ModelAttribute("pageModel") KuSysModel toolmodel,
	...
	public String view(@RequestParam("id") String id,
		...
		KuSys tool = kusysservice.findById(id);
		model.addAttribute("vo", tool);
		return "/duty/statellite/kusys/view";
	}
	...
	public String updateJump(
		...
		KuSys tool = kusysservice.findById(id);
		model.addAttribute("vo", tool);
		return "/duty/statellite/kusys/update";
	}
	...
	public String update(@ModelAttribute("pageModel") KuSysModel toolmodel,
			HttpServletRequest request, Model model,
			@RequestParam("id") String id) throws IOException {
		try {
			KuSys tool = kusysservice.findById(id);
			String checkid = tool.getCheckid().getId();
			EnvironmentEquipCheck enviroment = kusysservice.findByIden(checkid);
			// 设置当前用户为操作人
			User user = (User) request.getSession().getAttribute(
					SessionNames.LOGIN_USER);
			toolmodel.setOperator(user);
			kusysservice.update(tool, enviroment, toolmodel);
			model.addAttribute("returnStr",
					new ResultModel(true, "操作成功", null).toJson());
		} catch (Exception e) {
			model.addAttribute("returnStr",
	...
	public String delete(@ModelAttribute("pageModel") KuSysModel toolmodel,
			HttpServletRequest request, Model model,
		...
		try {
			...
			String checkid = tool.getCheckid().getId();
			EnvironmentEquipCheck enviroment = kusysservice.findByIden(checkid);
			kusysservice.delete(tool, enviroment);
			// 返回
		...
		return "duty/statellite/kusys/result";
	}
