...
<script>
	...
	function findFaultInfo() {
		...
		if (faultCompany == null || faultCompany == "") {
			alert("请填写事件所属单位");
			return;
		}
		$.ajax({
			type : "GET",
			url : "${rootUrl}faultmng/getFaultInfo.dox?faultCompany="
					+ encodeURI(encodeURI(faultCompany)),
			data : {
	...
	<div class="window"
		...
		<div class="mtab">
			...
			<form:form modelAttribute="pageModel" id="addForm" name="addForm"
				action="${rootUrl }faultmng/add.do" method="post">
			    ...
			    <tr>
					<th>申告所属单位</th>
					<td>
						<input type="text" id="faultCompany" name="faultCompany"/>
					</td>
					<th>事件类别</th>
				...
				<tr>
					<th>坐席电话</th>
					<td><input type="text" id="stationPhone" name="stationPhone"/></td>
					<th><label for="tags">事件发生地址</label></th>
					<td><input type="text" id="faultArea" name="faultArea"/></td>
				</tr>
				<tr>
					<th>是否正常咨询</th>
