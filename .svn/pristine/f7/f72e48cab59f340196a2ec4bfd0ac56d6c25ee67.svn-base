						...
						<div class="br">
							...
							<div id="lmsx">
								...
								<form:form modelAttribute="queryModel" id="listForm"
									name="listForm" action="${rootUrl}news/log/manage.do"
									method="post">
									<!-- 查询条件 -->
									<div class="boxbtn">
										<div class="info">
											...
											提交人：
											<form:input path="createUserEq" size="10" maxlength="30" />
											栏目：
											<form:select path="columnsEq" cssStyle="width:90px" >
												<form:option value=""></form:option>
												<form:options items="${newsColumnsEnum}" itemLabel="value"  />
											</form:select>
											操作人：
											<form:input path="optUserEq" size="10" maxlength="30" />
											提交时间：
											<input readonly="true" id="startOptTime" name="startOptTime" size="15" maxlength="18" class='Wdate' onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'endOptTime\')}'})"/>-
											<input readonly="true" id="endOptTime" name="endOptTime" size="15" maxlength="18" class='Wdate' onFocus="WdatePicker({minDate:'#F{$dp.$D(\'startOptTime\')}'})" />
										</div>
