...
<body>
	<div class="window">
		<dl class="title">
			<c:if test="${dgCheck.checkType eq 'MONTH' }">
				<dd>柴油发电机设备月检--更新</dd>
			</c:if>
			<c:if test="${dgCheck.checkType eq 'QUARTER' }">
				<dd>柴油发电机设备季检--更新</dd>
			</c:if>
			<c:if test="${dgCheck.checkType eq 'YEAR' }">
				<dd>柴油发电机设备年检--更新</dd>
			</c:if>
			<dt>
		...
		<div class="mtab">
			<form:form modelAttribute="pageModel" id="editForm" name="editForm" action="${rootUrl}duty/dgdutymonth/update.do" method="post">
				<input type="hidden" id="id" name="id" value="${dgCheck.id }"/>
