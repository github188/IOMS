		...
		<div class="mtab">
			<table width="100%">
				...
				<tr>
					...
					<c:if test="${tempCircuit.circuitType eq 'KUBROADCAST'}">
						<td style="text-align: left;"><c:forEach
								items="${kuBroadcastList}" var="vo" varStatus="voStatus">
								${vo.node}，
						</c:forEach></td>
					</c:if>
				...
				<c:if
					...
					<tr>
						<%-- <th>业务名称::</th>
						<td colspan="2" style="text-align: left;">${fn:escapeXml(tempcircuitDetail.businessName)}</td> --%>
						<th>电路名称:</th>
					...
					<tr>
						<th>节点:</th>
						<td style="text-align: left;">${tempcircuitDetail.aNodeNumber}</td>
						<th>槽位:</th>
					...
					<c:if test="${tempCircuit.circuitType eq 'KUBROADCAST'}">
						<c:forEach items="${kuBroadcastList}" var="vo"
							varStatus="voStatus">
							<tr>
								<th>节点:</th>
					...
					<c:if test="${tempCircuit.circuitType eq 'KUDATA'}">
						<tr>
							<th>节点:</th>
							<td style="text-align: left;">${tempcircuitDetail.bNodeNumber}</td>
							<th>槽位:</th>
					...
					<tr>
						<th>电路类型:</th>
						<td colspan="2" style="text-align: left;">${tempcircuitDetail.circuitType.name}</td>
						<th>业务速率（kbps）:</th>
						<td colspan="2" style="text-align: left;">${tempcircuitDetail.rate}</td>
					</tr>
					<tr>
						<th>备注信息:</th>
						<td colspan="5" style="text-align: left;"><textarea
