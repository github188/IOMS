...
<script>
	$(function() {
		...
		$("#submit").click(function() {
			...
			var checks = document.getElementsByName("nextHanleUser");
			n = 0;
	...
	<div class="window">
		...
		<div class="mtab">
			...
			<form:form id="addForm" name="addForm"
				action="${rootUrl }faultmng/fault/faultdept.do" method="post">
				<input id="faultId" name="faultId" value="${fault.id }"
					type="hidden" />
				<table width="100%">
					<tr>
						<th width="10%">处理人</th>
						<th width="10%" align="right">${currentHandleUser }</th>
						<th width="10%">技术工程师</th>
						<th width="70%"><c:forEach items="${users }" var="user">
								<c:if test="${user.name ne currentHandleUser && user.id ne fault.firstHandelUser.id}">
									<label><input type="checkbox" value="${user.name }"
										name="nextHanleUser" id="nextHanleUser" />${user.name }</label>
								</c:if>
							</c:forEach></th>
					</tr>
					<tr align="center">
