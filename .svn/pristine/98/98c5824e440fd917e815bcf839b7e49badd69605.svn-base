...
<tr>
	...
	<td width="8%"><fmt:formatDate value="${vo.planOpenTime.time}" type="date"
			pattern="yyyy-MM-dd" /></td>
	<td width="8%"><fmt:formatDate value="${vo.planEndTime.time}" type="date"
			pattern="yyyy-MM-dd" /></td>
	...
	<td class="tdbtn" width="10%">
		<c:if test="${!empty vo.engineerName}">
			<c:if test="${fn:indexOf(vo.engineerName,currentUser) > -1}">
				<a href="#" onclick="forcedelete('${vo.id}')">强制撤销</a>
			</c:if>
		</c:if>
	</td>
</tr>
