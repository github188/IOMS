...
<tr>
	...
	<td>
		<sec:authorize ifAnyGranted="${jspAuthorities['zjups_check_quarter_view']}">
			<a class="my_edit {boxwidth:1000,boxheight:700}" 
			   href="${rootUrl}duty/upsdutyquarter/zjups/view.do?id=${vo.id}">查看</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['zjups_check_quarter_del']}">
			<a href="${rootUrl}duty/upsdutyquarter/zjups/delete.do?id=${vo.id}"
				id="removeRecord" confirm_message="你确定要删除此记录？" class="my_remove">删除</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['zjups_check_quarter_update']}">
		<a href="${rootUrl}duty/upsdutyquarter/zjups/update.do?id=${vo.id}"
