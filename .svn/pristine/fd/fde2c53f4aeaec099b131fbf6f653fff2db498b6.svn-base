package cn.com.atnc.ioms.mng.activiti;
...
/**
 * 
 * 类说明：工作流中流程以及流程实例相关Service
 * 
 * @author: 局域网_王鹏
 * @time: 2014年4月18日下午7:16:06
 * @version:1.0
 */
@Service
public class WorkflowProcessDefinitionService {
	 ...
	 *             找不到zip文件时
	 */
	private void deploySingleProcess(ResourceLoader resourceLoader,
			String processKey, String exportDir) throws IOException {
		String classpathResourceUrl = "classpath:/deployments/" + processKey
				+ ".bar";
		logger.debug("read workflow from: {}", classpathResourceUrl);
		Resource resource = resourceLoader.getResource(classpathResourceUrl);
		InputStream inputStream = resource.getInputStream();
		if (inputStream == null) {
			logger.warn("ignore deploy workflow module: {}",
