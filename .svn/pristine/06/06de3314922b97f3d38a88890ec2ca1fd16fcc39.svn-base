...
<tr>
	...
	<td>
		<sec:authorize ifAnyGranted="${jspAuthorities['txjdups_check_year_view']}">
			<a class="my_edit {boxwidth:1000,boxheight:700}" 
			   href="${rootUrl}duty/upsdutyyear/txjdups/view.do?id=${vo.id}">查看</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['txjdups_check_year_del']}">
			<a href="${rootUrl}duty/upsdutyyear/txjdups/delete.do?id=${vo.id}"
				id="removeRecord" confirm_message="你确定要删除此记录？" class="my_remove">删除</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['txjdups_check_year_update']}">
		<a href="${rootUrl}duty/upsdutyyear/txjdups/update.do?id=${vo.id}"
