	...
	<div class="window">
		<dl class="title">
			<c:if test="${accCheck.checkType eq 'MONTH' }">
				<dd>传输设备与通讯线路月检--更新</dd>
			</c:if>
			<c:if test="${accCheck.checkType eq 'QUARTER' }">
				<dd>传输设备与通讯线路季检--更新</dd>
			</c:if>
			<c:if test="${accCheck.checkType eq 'YEAR' }">
				<dd>传输设备与通讯线路年检--更新</dd>
			</c:if>
			<dt>
		...
		<div class="mtab">
			<form:form modelAttribute="pageModel" id="editForm" name="editForm" action="${rootUrl}duty/psotduty/txjdotical/update.do" method="post">
				<input type="hidden" id="id" name="id" value="${psOtCheck.id }"/>
