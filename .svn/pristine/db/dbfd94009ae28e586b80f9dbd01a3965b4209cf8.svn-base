	...
	<div class="window">
		...
		<div class="mtab">
				<form:form modelAttribute="pageModel" id="networkForm"
					name="networkForm" action="${rootUrl }transfernet/deal.do" method="post">
					<%@include file="innerDetail.jsp"%>
					<form:hidden path="transferNet.id" />
					<form:hidden path="pid" />
					<input type="hidden" name="fid" value="${m.transferNet.id}" />
					<input type="hidden" id="stat" value="${m.transferNet.approveStat}" />
					...
					<table width="100%" style="margin: 0px auto">
							<c:if test="${fn:contains(role, '|csLeader|')}">
								<c:if test="${m.transferNet.approveStat eq 'csleader_approve' }">
									 <tr>
							...
							<c:if test="${fn:contains(role, '|atncLeader|')}">
							    <c:if test="${m.transferNet.approveStat eq 'atncleader_firstdeal'}">
							    <tr class="transfer">
							    ...
							    <c:if test="${m.transferNet.approveStat eq 'atncleader_seconddeal'}">
							        <tr class="transfer">
								...
								<c:if test="${m.transferNet.approveStat eq 'atncleader_thirddeal'}">
									<tr class="transfer">
										<th width="20%" align="right">请选择审核工程师：<span style="color: red">(*)</span></th>
										<td colspan="6">
											<form:select path="auditEngineer" cssStyle="width:35%">
							    ...
							    <c:if test="${m.transferNet.approveStat eq 'atncleader_approve'}">
							        <tr id="chooseflag">
									    <th width="40%" align="right">请选择审批类别：<span style="color: red">(*)</span>
									    </th>
									        <td width="60%" colspan="6">
							        ...
							        <tr id = "checkapply">
										<th width="40%" align="right">请选择是否同意(用户申请)：<span style="color: red">(*)</span></th>
										<td width="60%" colspan="6">
											<form:radiobutton path="apply" class="apply" value="1"/>同意用户申请
											<form:radiobutton path="apply" class="apply" value="0" checked="checked"/>不同意用户申请
										</td>
							...
							<c:if test="${fn:contains(role, '|engineer|')}">
							    <c:if test="${m.transferNet.approveStat eq 'engineer_config'}">
								<tr>
							    ...
							    <c:if test="${m.transferNet.approveStat eq 'engineer_transfer'}">
							    </c:if>
							    <c:if test="${m.transferNet.approveStat eq 'engineer_secondaudit'}">
								<tr id="chooseflag">
							...
							<c:if test="${fn:contains(role, '|netmanager|')}">
							 <c:if test="${m.transferNet.approveStat eq 'netmanager_approve'}">
							     <tr id = "checkapply">
										<th width="20%" align="right">请选择是否同意(用户申请)：<span style="color: red">(*)</span></th>
										<td width="60%" colspan="6">
											<form:radiobutton path="apply" class="apply" value="1"/>同意用户申请
							 ...
							 <c:if test="${m.transferNet.approveStat eq 'generalmanager_approve'}">
							     <tr id = "checkapply">
										<th width="20%" align="right">请选择是否同意(用户申请)：<span style="color: red">(*)</span></th>
										<td width="60%" colspan="6">
											<form:radiobutton path="apply" class="apply" value="1"/>同意用户申请
						...
						<div class="btn">
							<c:choose>
								...
								<c:when test="${role == 'engineer'}">
									<c:choose>
										<c:when test="${m.transferNet.approveStat eq 'engineer_firstaudit' ||m.transferNet.approveStat eq 'engineer_secondaudit'}">
