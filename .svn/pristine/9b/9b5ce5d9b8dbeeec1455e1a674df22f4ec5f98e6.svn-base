 ...
 * @date 2015年10月15日 上午11:02:26
 * @since 1.0.0
 */
public enum FormalStatEnum {
	...
	unsubmitted("待客户提交"), returned("已退回"), areaexaminer_approve("地区网络中心审批"), areaexaminer_affirm(
			...
			"天航信配置阶段_技术工程部领导安排配置工程师"), engineer_firstaudit(
			"天航信审核阶段_工程师审核用户提交材料"), engineer_config("天航信配置阶段_配置工程师配置"), atncleader_thirddeal(
			"天航信配置阶段_技术工程部领导安排审核工程师"), engineer_secondaudit(
	...
	private FormalStatEnum(String value) {
		this.value = value;
	}
	...
	public static FormalStatEnum getApproveStatEnumByOrdinal(int value) {
		for (FormalStatEnum type : FormalStatEnum.values()) {
	...
	public static FormalStatEnum getApproveStatEnumByValue(String value) {
		for (FormalStatEnum type : FormalStatEnum.values()) {
