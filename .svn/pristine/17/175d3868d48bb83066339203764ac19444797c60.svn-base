package cn.com.atnc.ioms.web.duty.satellitedayauto.slh;
 ...
 */
@Controller
@RequestMapping("/duty/satellitedayauto/slhpes/")
public class SlhPesController extends MyBaseController {
	...
	public String addJump(@ModelAttribute("pageModel") SlhPesModel pageModel,
			@RequestParam("optType") String optType, Model model) {
		// 检查结果枚举类
		model.addAttribute("optType", optType);
		// 跳转添加页面
		return "duty/satellitedayauto/slhpes/add";
	}
	...
	public String add(@ModelAttribute("pageModel") SlhPesModel pageModel,
		...
		try {
			...
			// 保存新数据
			satSlhPesService.saveOrUpdate(pageModel);
	...
	public String delete(@ModelAttribute("pageModel") SlhPesModel pageModel,
			HttpServletRequest request, Model model) throws IOException {
		try {
			// 删除
			satSlhPesService.delete(pageModel);
			// 返回
			// 返回
		...
		return "duty/satellitedayauto/slhpes/result";
	}
	...
	public String updateJump(
		...
		// 检查结果枚举类
		Pagination pagn = satSlhPesService.queryPageByModel(queryModel);
		// 取出列表
		...
		return "duty/satellitedayauto/slhpes/update";
	}
	...
	public String update(@ModelAttribute("pageModel") SlhPesModel pageModel,
			HttpServletRequest request, Model model) throws IOException {
		try {
			// 设置当前用户为操作人
			User user = (User) request.getSession().getAttribute(
