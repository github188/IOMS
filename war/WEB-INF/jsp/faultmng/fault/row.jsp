...
<c:if test="${vo.faultStatus ne 'HANDLEOA'}">
...
<tr>
	...
	<%-- <td>${vo.faultNumber}<input type="hidden" name="faultId"
		value="${vo.id}" /></td> --%>
	<td id="faultManage-${vo.faultNumber }"><span class='folder'>
		<a href="${rootUrl }faultmng/fault/view.do?faultId=${vo.id}"
		class="my_view {boxwidth:750,boxheight:500}">${vo.faultNumber}</a>
		</span>
			...
			<c:if test="${vo.faultStatus ne 'ASSIGNOK' || ( vo.faultStatus eq 'ASSIGNOK' && vo.hasHandleUser eq true )}">
				...
				<c:if test="${vo.faultStatus eq 'WAIT'}">
					<a href="${rootUrl }phonemng/callcenter/faulthandle.do?faultId=${vo.id}"
						class="my_edit {boxwidth:750,boxheight:700}">&nbsp;处理&nbsp;</a>
				...
				<c:if test="${vo.faultStatus eq 'SUCCESS' && optType eq 'COMPLETEFAULT'}">
					<a href="${rootUrl }faultmng/fault/view.do?faultId=${vo.id}"
						class="my_view {boxwidth:750,boxheight:850}">&nbsp;详情&nbsp;</a>
				</c:if>
				<c:if test="${vo.faultStatus eq 'SUCCESS' && optType eq 'EVALUATE'}">
					<a href="${rootUrl }faultmng/fault/evaluate.do?faultId=${vo.id}"
						class="my_view {boxwidth:750,boxheight:850}">&nbsp;评价&nbsp;</a>
				</c:if>
				...
				<c:if test="${vo.taskName eq '坐席值班员处理' }"><!-- ||  vo.taskName eq '其他部门处理' -->
				 ...
				 <c:if test="${vo.source ne 'OFFPOPUP' && vo.faultStatus ne 'SUCCESS' }">
					...
					<!-- 第一处理人和管理员可以修改故障单 -->
					<c:if test="${(vo.firstHandelUser.name eq currentHandleUser || currentHandleUser eq '管理员') && vo.faultStatus ne 'DELETE' && vo.source ne 'KUALARM'}">
					...
					<c:if test="${currentHandleUser eq '管理员' && vo.faultStatus ne 'WAIT' && vo.faultStatus ne 'DELETE' && vo.source ne 'KUALARM'}">
