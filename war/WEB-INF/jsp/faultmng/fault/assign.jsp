...
<body style="overflow-x: hidden">
	...
	<div class="window">
		<dl class="title">
			<dd>事件分配</dd>
			<dt>
				<a href="#none" class="close"><img src="${rootUrl }images/close.png" alt="关闭" /></a>
			</dt>
		</dl>
		...
		<div class="mtab">
			...
			<table width="100%">
				...
				<c:forEach items="${faultHandles}" var="handles" varStatus="voStatus">
					<c:if test="${handles.handleType ne 'FAULTTEMPRARY' }">
						<tr>
						    <td><fmt:formatDate value="${handles.operatTime }" type="date" pattern="yyyy-MM-dd HH:mm:ss" /></td>
			...
			<form:form id="addForm" name="addForm" action="${rootUrl }faultmng/fault/assign.do" method="post">
				<input id="faultId" name="faultId" value="${fault.id }" type="hidden" />
				<table width="100%" >
					<tr>
						<th width="10%">处理人</th>
						<th width="15%" align="right">${currentHandleUser }</th>
						<th width="15%">值班工程师</th>
						<th width="60%">
							<select id="nextHanleUser" name="nextHanleUser" style="width: 133px;height:22px;">
									 <c:forEach items="${users}" var="user" varStatus="voUsers">
										 <c:if test="${user.id ne fault.firstHandelUser.id}">
