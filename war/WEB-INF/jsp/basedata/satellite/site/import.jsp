	...
	<div class="window" style="min-height: 620px;">
		...
		<div>
			<form:form name="myForm" method="post" enctype="multipart/form-data"
				...
				action="${rootUrl}basedata/satellite/site/impexcel.do" id="myForm">
				<c:if test="${empty failedData}">
					<table width="96%">
						<tr>
							<td align="center"><font color="red">*</font>文件路径:</td>
							<td align="left""><input class="a-upload" type="file"
								id="localFilePath" name="localFilePath" style="width: 500px"
		...
		<c:if test="${not empty failedData}">
			<div class="mtab">
				<h1>导入失败记录</h1>
				<table style="width: 96%; table-layout: fixed;" border="0"
							...
							<c:otherwise>
								<tr class="info_in">
									<c:forEach items="${data}" var="oneData" varStatus="dataIndex">
										<td style="word-wrap: break-word;" class="right_td">${oneData}&nbsp;</td>
									</c:forEach>
								</tr>
							</c:otherwise>
						</c:choose>
			...
			<div class="boxbtn">
				<a href="#" id="upRtn" name="upRtn" class="btn"
					onclick="javascript:window.location.href='${rootUrl }basedata/satellite/site/impexcel.do'">返回</a>
			</div>
		</c:if>
		<br />
	</div>
