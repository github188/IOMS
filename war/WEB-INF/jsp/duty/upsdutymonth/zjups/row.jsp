...
<tr>
	...
	<c:forEach items="${vo.zgvZlcMeasure }" var="zzm">
		<td>${zzm.zgvInputMeasure }</td>
		<td>${zzm.zgvInputResult }</td>
		<td>${zzm.zgvOutputMeasure }</td>
		<td>${zzm.zgvOutputResult }</td>
		<td>${zzm.zlcInputMeasure }</td>
		<td>${zzm.zlcInputResult }</td>
		<td>${zzm.zlcOutputMeasure }</td>
	...
	<td>
		<sec:authorize ifAnyGranted="${jspAuthorities['zjups_check_month_view']}">
			<a class="my_edit {boxwidth:1000,boxheight:700}" 
			   href="${rootUrl}duty/upsdutymonth/zjups/view.do?id=${vo.id}">查看</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['zjups_check_month_del']}">
			<a href="${rootUrl}duty/upsdutymonth/zjups/delete.do?id=${vo.id}"
				id="removeRecord" confirm_message="你确定要删除此记录？" class="my_remove">删除</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['zjups_check_month_update']}">
		<a href="${rootUrl}duty/upsdutymonth/zjups/update.do?id=${vo.id}"
