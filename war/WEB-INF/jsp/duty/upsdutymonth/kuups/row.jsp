...
<tr>
	...
	<td>
		<sec:authorize ifAnyGranted="${jspAuthorities['kuups_check_month_view']}">
			<a class="my_edit {boxwidth:1000,boxheight:700}" 
			   href="${rootUrl}duty/upsdutymonth/kuups/view.do?id=${vo.id}">查看</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['kuups_check_month_del']}">
			<a href="${rootUrl}duty/upsdutymonth/kuups/delete.do?id=${vo.id}"
				id="removeRecord" confirm_message="你确定要删除此记录？" class="my_remove">删除</a>
		</sec:authorize>
		<sec:authorize ifAnyGranted="${jspAuthorities['kuups_check_month_update']}">
		<a href="${rootUrl}duty/upsdutymonth/kuups/update.do?id=${vo.id}"
