...
<div style="margin: 10px 20px 0px; padding: 0px">
	<dl class="title">
		<dd>十里河8830(${atmDayMgx.area.value})</dd>
	</dl>
</div>
<table width="100%" border="1">
	...
	<tr>
		...
		<td colspan="2">与网控中心8850节点的连通性</td>
		<td><c:if test="${view eq 1 or audit eq 1}">
                ${atmDayMgx.wkzx8850_con.value}
            </c:if> <c:if test="${view eq 0}">
                <select id="wkzx8850_con" name="wkzx8850_con">
                    <option value="${atmDayMgx.wkzx8850_con }">${atmDayMgx.wkzx8850_con.value }</option>
                    <c:forEach items="${statusResults}" var="vo">
                        <c:if test="${atmDayMgx.wkzx8850_con.value != vo.value }">
                            <option value="${vo}">${vo.value}</option>
                        </c:if>
                    </c:forEach>
                </select>
	...
	<tr>
		...
		<td colspan="2"><c:if test="${view eq 1 or audit eq 1}">
				...
				<textarea id="slhsbz" name="slhsbz" rows="5" class="tdWidth" 
