...
<tr>
    ...
    <!-------------与网控中心8850节点的连通性------------>
    <td><c:choose>
            <c:when test="${vo.wkzx8850_con eq 'UNNORMAL'}">
                <font>${vo.wkzx8850_con.value}</font>
            </c:when>
            <c:otherwise>
                        ${vo.wkzx8850_con.value}
                    </c:otherwise>
        </c:choose></td>
    <!------------- 中继连通状态 ------------->
    <td><c:choose>
    ...
    <!-- 其他信息 -->
    <td><sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_slhs_view']}">
            <a class="my_edit {boxwidth:1000,boxheight:700}"
                href="${rootUrl}duty/atmday/mgx/txjd8850/view.do?id=${vo.id}&tabType=SLH_8830">查看</a>
        </sec:authorize> <sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_slhs_update']}">
            <a href="${rootUrl}duty/atmday/mgx/update.do?id=${vo.id}&tabType=SLH_8830"
                class="my_edit {boxwidth:1000,boxheight:700}">修改</a>
        </sec:authorize> <sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_slhs_audit']}">
            <c:if
                test="${vo.creater.name ne currentUser.name and empty vo.auditer}">
                <a class="my_edit {boxwidth:1000,boxheight:700}"
                    href="${rootUrl}duty/atmday/mgx/audit.do?id=${vo.id}&tabType=SLH_8830">审核</a>
            </c:if>
        </sec:authorize> <sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_slhs_del']}">
            <a href="${rootUrl}duty/atmday/mgx/delete.do?id=${vo.id}&tabType=SLH_8830"
