...
<tr>
    ...
    <!-- 其他信息 -->
    <td><sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_txjds_view']}">
            <a class="my_edit {boxwidth:1000,boxheight:700}"
                href="${rootUrl}duty/atmday/mgx/txjd8850/view.do?id=${vo.id}&tabType=TXJD_8830">查看</a>
        </sec:authorize> <sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_txjds_update']}">
            <a href="${rootUrl}duty/atmday/mgx/update.do?id=${vo.id}&tabType=TXJD_8830"
                class="my_edit {boxwidth:1000,boxheight:700}">修改</a>
        </sec:authorize> <sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_txjds_audit']}">
            <c:if
				test="${vo.creater.name ne currentUser.name and empty vo.auditer}">
				<a class="my_edit {boxwidth:1000,boxheight:700}"
					href="${rootUrl}duty/atmday/mgx/audit.do?id=${vo.id}&tabType=TXJD_8830">审核</a>
			</c:if>
        </sec:authorize> <sec:authorize
            ifAnyGranted="${jspAuthorities['atm_check_txjds_del']}">
            <a href="${rootUrl}duty/atmday/mgx/delete.do?id=${vo.id}&tabType=TXJD_8830"
