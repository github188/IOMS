...
<tr>
	...
	<td><c:if
			...
			<sec:authorize
				...
				<a
					href="${rootUrl}duty/satellite/day/tes/audit.do?id=${vo.id}&optType=txjddeccon"
			...
			<a class="my_edit {boxwidth:1000,boxheight:600}"
				href="${rootUrl}duty/satellite/day/tes/view.do?id=${vo.id}&optType=txjddeccon">查看</a>
		</sec:authorize> <sec:authorize
			ifAnyGranted="${jspAuthorities['sat_check_day_tes_del']}">
			<a
				href="${rootUrl}duty/satellite/day/tes/delete.do?id=${vo.id}&optType=txjddeccon"
				id="removeRecord" confirm_message="你确定要删除此记录？" class="my_remove">删除</a>
		</sec:authorize> <sec:authorize
			ifAnyGranted="${jspAuthorities['sat_check_tes_day_deccon_update']}">
			<a
				href="${rootUrl}duty/satellite/day/tes/tesdeccon/update.do?id=${vo.id}&optType=txjddeccon"
