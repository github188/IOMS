...
<tr>
	...
	<td class="tdbtn" width="14%"><a
		...
		class="my_add{boxwidth:850,boxheight:500}">日志</a> <c:if
			...
			test="${vo.circuitType=='TESDATA' || vo.circuitType=='TESBROADCAST'}">
	...
	<td class="tdbtn" width="10%"><c:if
			test="${vo.circuitType=='FRATM'}">
			<a
				href="${rootUrl}tempcircuit/atmfratm/mod.do?tempcircuitid=${vo.id}"
				class="my_edit {boxwidth:650,boxheight:500}">修改配置</a>
		</c:if> <c:if test="${vo.circuitType=='MPLSVPN'}">
			<a
				href="${rootUrl}tempcircuit/atmmplsvpn/mod.do?tempcircuitid=${vo.id}"
				class="my_edit {boxwidth:650,boxheight:500}">修改配置</a>
		</c:if>
			 <c:if
			test="${vo.circuitType=='TESDATA' || vo.circuitType=='TESBROADCAST'}">
			<a href="${rootUrl}tempcircuit/tesdb/mod.do?tempcircuitid=${vo.id}"
				class="my_edit {boxwidth:650,boxheight:500}">修改配置</a>
		</c:if> <c:if test="${vo.circuitType=='TESVOICE'}">
			<a
				href="${rootUrl}tempcircuit/tesaudio/mod.do?tempcircuitid=${vo.id}"
				class="my_edit {boxwidth:650,boxheight:500}">修改配置</a>
		</c:if> <c:if test="${vo.circuitType=='KUDATA'|| vo.circuitType=='KUBROADCAST'}">
			<a href="${rootUrl}tempcircuit/ku/mod.do?tempcircuitid=${vo.id}"
				class="my_edit {boxwidth:650,boxheight:500}">修改配置</a>
		</c:if></td>
</tr>
