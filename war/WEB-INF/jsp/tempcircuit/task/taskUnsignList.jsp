...
<script type="text/javascript">
	...
	function claim(taskid, tempcircuitid) {
		if (confirm("您确定签收该任务？")) {
			var url = rootUrl + "tempcircuit/claim.do?";
			$.ajax({
				url : url,// 需要链接到服务器地址
				type : 'GET',
				data : {
...
<body>
	<table id="dataUnSingTable" width="100%" border="0" cellspacing="0"
		cellpadding="2">
		<thead>
			<tr>
				<th>电路类型</th>
		...
		<tbody>
			<c:forEach items="${unSignTaskList}" var="tempcircuit">
				...
				<tr id="${tempcircuit.id }" tid="${tempcircuit.id }">
					...
					<td class="tdbtn"><c:if
							...
							<a
								...
								class="my_view{boxwidth:850,boxheight:500}">电路申请信息</a>
						</c:if> <c:if test="${tempcircuit.circuitType  =='TESDATA'  || tempcircuit.circuitType=='TESBROADCAST'}">
							<a
							...
							<a
								...
								class="my_view{boxwidth:850,boxheight:500}">电路申请信息</a>
						</c:if> <c:if test="${tempcircuit.circuitType  =='KUDATA'|| tempcircuit.circuitType=='KUBROADCAST'}">
							<a
					...
					<td class="tdbtn"><a
						onclick="claim('${task.id}','${tempcircuit.id}')" href="#">签收</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
