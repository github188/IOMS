...
<tr>
	...
	<c:choose>
	    <c:when test="${fn:escapeXml(vo.alarmStatus.value) eq '异常'}">
	        <sec:authorize ifAnyGranted="${jspAuthorities['linkalarm_view']}">
		       <a href="${rootUrl }maintain/alarminfo/linkalarmview.do?id=${vo.equipId.id}"
			    class="my_view {boxwidth:850,boxheight:300}">查看</a>
	        </sec:authorize> 
	    </c:when>
	</c:choose>
	<sec:authorize ifAnyGranted="${jspAuthorities['linkalarm_history']}">
		<a href="${rootUrl }maintain/alarminfo/linkalarmhistory.do?equipId=${vo.equipId.id}"
