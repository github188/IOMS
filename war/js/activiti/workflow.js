						...
						if ($('#workflowTraceDialog').length == 0) {
							$(
									'<div/>',
									{
										...
										html : "<div><img src='"
												+ imageUrl
												+ "' style='position:absolute; left:5px; top:55px;' />"
												+ "<div id='processImageBorder'>"
												+ positionHtml + "</div>"
												+ "</div>"
									}).appendTo('body');
...
function graphTempCircuitMngTrace(options) {
	var _defaults = {
		srcEle : this,
		pid : $(this).attr('pid')
	};
						...
						if ($('#workflowTraceDialog').length == 0) {
							$(
									'<div/>',
									{
										...
										html : "<div><img src='"
												+ imageUrl
												+ "' style='position:absolute; left:75px; top:55px;' />"
												+ "<div id='processImageBorder'>"
												+ positionHtml + "</div>"
												+ "</div>"
									}).appendTo('body');
...
function getTempCircuitMngTrace(pid) {
	// 获取图片资源
	var imageUrl = rootUrl + "workflow/resource/process-instance.dox?pid="
			+ pid + "&type=image";
	$
