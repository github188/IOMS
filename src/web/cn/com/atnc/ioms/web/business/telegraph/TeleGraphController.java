...
public class TeleGraphController extends MyBaseController {
	 ...
	 * 页面初始化
	 * @author WangLingbin
	 * @date  2015年5月28日下午3:27:40
	 * @since 1.0.0
	 */
	@RequestMapping(value = "tasklist.do", method = RequestMethod.GET)
	public String manage(HttpServletRequest request, Model model) {
		super.pageInfo(model, request);
		return "business/telegraph/manage";
	}

	/**
	 * 加载数据，返回前台显示
	 * @author WangLingbin
	 * @date  2015年5月28日下午3:27:50
	 * @since 1.0.0
	 */
	...
	public String manageList(@ModelAttribute("queryModel") TeleGraphQueryModel queryModel,
		...
		if (roleNames.toString().contains("|"+RoleEnum.csLeader.name()+"|")){ // 客服领导
			...
			model.addAttribute("pagn", teleGraphService.queryPageByModel(queryModel));
		}
		...
		return "business/telegraph/queryResult";
	}
	...
	public String ajaxLoad(@ModelAttribute("queryModel") TeleGraphQueryModel queryModel,
		...
		if (roleNames.toString().contains("|"+RoleEnum.csLeader.name()+"|")){ // 客服领导
			...
			model.addAttribute("returnStr",
					JacksonTools.toJsonStr(teleGraphService.queryPageByModel(queryModel)));
		}
		if(roleNames.toString().contains("|"+RoleEnum.csStaff.name()+"|")){ // 客服专员
	...
	public String setupDetailForm(@RequestParam("id")String id,HttpServletRequest request, Model model) {
		...
		//根据父类ID获取TeleGraphApply实体集合
		List<TeleGraphApply> teleGraphApplys = teleGraphApplyService.getApplysByParentId(id);
		model.addAttribute("teleGraphApplys",teleGraphApplys);
		//获取电报配置实体集合
		List<TeleGraphConfig> teleGraphConfigs = teleGraphConfigService.getConfigsByTeleGraphId(id);
		model.addAttribute("teleGraphConfigs", teleGraphConfigs);
		//上传附件
		...
		return "business/telegraph/detail";
	}
	...
	public String draftGet(@RequestParam("id") String id,@ModelAttribute("pageModel") TeleGraphQueryModel queryModel,
		...
		model.addAttribute("attaches", attaches);
		model.addAttribute("teleAttachTypeEnums", teleAttachTypeEnums);
		return "business/telegraph/draft";
	}
	...
	public String draftPost(@RequestParam("id") String id,@ModelAttribute("pageModel") TeleGraphQueryModel queryModel,
		...
		return "business/telegraph/result";
	}
	...
	public String setupDealForm(@RequestParam("id")String id,
		...
		if(roleNames.toString().contains("|"+RoleEnum.engineer.name()+"|")) {    // 工程师
			...
			//model.addAttribute("status", teleGraph.getApproveStat().getValue());
		}
		if(roleNames.toString().contains("|"+RoleEnum.atncLeader.name()+"|")) {	//网控领导
			model.addAttribute("engineers",engineers);
		...
		model.addAttribute("role", roleNames);
		return "business/telegraph/deal";
	}
	...
	public String peocessDealForm(HttpServletRequest request, Model model,
			...
			@RequestParam(value = "identifier", required = false) List<String> identifiers,
			@RequestParam(value = "atncRountAddr", required = false) List<String> atncRountAddrs,
			@RequestParam(value = "atncConfigPeople", required = false) List<String> atncConfigPeoples,
			@RequestParam(value = "atncConnectionCondition", required = false) List<String> atncConnectionConditions,
			@RequestParam(value = "allocatedAddr", required = false) List<String> allocatedAddrs,
			@RequestParam(value = "operPeople", required = false) List<String> operPeoples,
			@RequestParam(value = "operTime", required = false) List<String> operTimes,
			@RequestParam(value = "originalAddr", required = false) List<String> originalAddrs) {
		try{
			//如果为工程师测试阶段，则完善转报申请信息中天航信公司应填写的记录
			if(!CollectionUtils.isEmpty(allocatedAddrs)) {
				teleGraphApplyService.engineerTest(queryModel, allocatedAddrs, operPeoples, operTimes);
			}
			//当前登录用户
			User user =  this.getCurrentUser(request);
			//处理审批操作
			TeleGraph teleGraph = teleGraphService.dealForm(request, queryModel,user, originalAddrs);
			//如果为配置阶段，则保存配置信息
			if(queryModel.isApproveStat()){		//操作类别（true：配置/false:移交）
				if(!CollectionUtils.isEmpty(atncRountAddrs)) {
					teleGraphConfigService.config(queryModel, identifiers,
							atncRountAddrs, atncConfigPeoples, atncConnectionConditions);
				}
			}
			addSuccess(model, "处理成功！");
			//添加操作日志
			super.addLog(request, "客服专员处理成功(联系人:" + teleGraph.getApplyBaseInfo().getApplyLinkman()+")");
		}catch (Exception e) {
			e.printStackTrace();
	...
	public void downloadReturnedReport(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if(StringUtils.equalsIgnoreCase(fileExt, JasperReportUtils.getReportExt())) {
				...
				this.doDownLoad(request, response, data, ReturnedReportsEnum.apply.getValue());
			} else {
	...
	public ModelAndView previewApply(HttpServletRequest request,
		...
		Map<String, Object> param = teleGraphService.getReportParams(request, id, JasperReportsEnum.apply.toString());
		param.put("format", "pdf");
		mav = new ModelAndView(JasperReportsEnum.apply.toString(),param);
		//添加操作日志
	...
	*/
	...
	public ModelAndView previewConfig(HttpServletRequest request,
			ModelAndView mav,
			HttpServletResponse response, @RequestParam("id") String id) {
		Map<String, Object> param = teleGraphService.getReportParams(request, id, JasperReportsEnum.config.toString());
		param.put("format", "pdf");
		mav = new ModelAndView(JasperReportsEnum.config.toString(),param);
		//添加操作日志
