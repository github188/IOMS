 ...
 */
@Controller
@RequestMapping("/duty/statellite/cir/")
public class CirController extends MyBaseController {
	...
	public String addJump(@ModelAttribute("pageModel") Cir queryModel,
		...
		return "duty/statellite/cir/add";
	}
	...
	public String add(@ModelAttribute("pageModel") CirModel cirmodel,
		...
		try {
			...
			cirmodel.setOperator(user);
			model.addAttribute("currentUser", user);
			// 设置当前卫星【月检】【季检】【年检】类型
			cirmodel.setCheckType(checkType);
			// 将记录保存至数据库
			cirservice.save(cirmodel);
	...
	public String view(@RequestParam("id") String id,
		...
		Cir cir = cirservice.findById(id);
		model.addAttribute("vo", cir);
		return "/duty/statellite/cir/view";
	}
	...
	public String updateJump(
			...
			@ModelAttribute("pageModel") Cir pagemodel,
			HttpServletRequest request, Model model) {

		Cir cir = cirservice.findById(id);
		model.addAttribute("cir", cir);
		return "/duty/statellite/cir/update";
	}
	...
	public String update(@ModelAttribute("pageModel") CirModel cirmodel,
			HttpServletRequest request, Model model,
			@RequestParam("id") String id) throws IOException {
		try {
			Cir cir = cirservice.findById(id);
			String checkid = cir.getCheckid().getId();
			EnvironmentEquipCheck enviroment = cirservice.findByIden(checkid);
			System.err.println("enviroment---->" + enviroment);
			// 设置当前用户为操作人
			User user = (User) request.getSession().getAttribute(
					SessionNames.LOGIN_USER);
			cirmodel.setOperator(user);
			cirservice.update(cir, enviroment, cirmodel);
			model.addAttribute("returnStr",
					new ResultModel(true, "操作成功", null).toJson());
		} catch (Exception e) {
			model.addAttribute("returnStr",
	...
	public String delete(@ModelAttribute("pageModel") CirModel cirmodel,
			HttpServletRequest request, Model model,
		...
		try {
			...
			String checkid = cir.getCheckid().getId();
			EnvironmentEquipCheck enviroment = cirservice.findByIden(checkid);
			cirservice.delete(cir, enviroment);
			// 返回
		...
		return "duty/statellite/cir/result";
	}
}
