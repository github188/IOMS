 ...
 */
@Controller
@RequestMapping("/duty/statellite/tescheck/")
public class TesCheckController extends MyBaseController {
	@Autowired
	private TesCheckService tescheckdservice;
	...
	public String addJump(@ModelAttribute("pageModel") TesCheck queryModel,
		...
		return "duty/statellite/tescheck/add";
	}
	...
	public String add(@ModelAttribute("pageModel") TesCheckModel toolmodel,
		...
		try {
			...
			// 将记录保存至数据库
			tescheckdservice.save(toolmodel);
	...
	public String view(@RequestParam("id") String id,
		...
		TesCheck tool = tescheckdservice.findById(id);
		model.addAttribute("vo", tool);
		return "/duty/statellite/tescheck/view";
	}
	...
	public String updateJump(
			...
			@ModelAttribute("pageModel") TesCheckModel pagemodel,
			HttpServletRequest request, Model model) {

		TesCheck tool = tescheckdservice.findById(id);
		model.addAttribute("vo", tool);
		return "/duty/statellite/tescheck/update";
	}
	...
	public String update(@ModelAttribute("pageModel") TesCheckModel toolmodel,
			HttpServletRequest request, Model model,
			@RequestParam("id") String id) throws IOException {
		try {
			TesCheck tool = tescheckdservice.findById(id);
			String checkid = tool.getCheckid().getId();
			EnvironmentEquipCheck enviroment = tescheckdservice
					.findByIden(checkid);
			// 设置当前用户为操作人
			User user = (User) request.getSession().getAttribute(
					SessionNames.LOGIN_USER);
			toolmodel.setOperator(user);
			tescheckdservice.update(tool, enviroment, toolmodel);
			model.addAttribute("returnStr",
					new ResultModel(true, "操作成功", null).toJson());
		} catch (Exception e) {
			model.addAttribute("returnStr",
	...
	public String delete(@ModelAttribute("pageModel") TesCheckModel toolmodel,
			HttpServletRequest request, Model model,
		...
		try {
			...
			EnvironmentEquipCheck enviroment = tescheckdservice
					.findByIden(checkid);
			tescheckdservice.delete(tool, enviroment);
			// 返回
		...
		return "duty/statellite/tescheck/result";
	}
