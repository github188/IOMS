 ...
 */
@Controller
@RequestMapping("/duty/satellitedayauto/tesocc/")
public class TesOccController extends MyBaseController {

	@Autowired
	private SatTxjdTesOCCService satTxjdTesOCCService;
	...
	public String addJump(
			@ModelAttribute("pageModel") SatTxjdTesOccModel satTxjdTesOccModel,
			@RequestParam("optType") String optType, Model model) {
		// 检查结果枚举类
		model.addAttribute("optType", optType);
		// 跳转添加页面
		return "duty/satellitedayauto/txjdtesocc/add";
	}
	...
	public String add(
		...
		try {
			// 存储创建人
			satTxjdTesOccModel.setOperator(user);
			// 保存新数据
			satTxjdTesOCCService.saveOrUpdate(satTxjdTesOccModel);
	...
	public String delete(
		...
		try {
			// 删除
			satTxjdTesOCCService.delete(satTxjdTesOccModel);
			// 返回
		...
		return "duty/satellitedayauto/txjdtesocc/result";
	}
	...
	public String updateJump(
		...
		// 检查结果枚举类
		Pagination pagn = satTxjdTesOCCService.queryPageByModel(queryModel);
		// 取出列表
		...
		return "duty/satellitedayauto/txjdtesocc/update";
	}
	...
	public String update(
		...
		try {
			...
			satTxjdTesOccModel.setOperator(user);
			// 保存新数据
			satTxjdTesOCCService
					.saveOrUpdate(satTxjdTesOccModel);
