 ...
 * @since 1.0.0
 */
public enum TransferNetStatEnum {
	...
	unsubmitted("待客户提交"), returned("已退回"), areaexaminer_approve("地区网络中心审批"), areaexaminer_affirm(
			...
			"天航信审核阶段_工程师审核用户提交材料"), engineer_config("天航信配置阶段_配置工程师进行配置"), engineer_secondaudit(
	...
	private TransferNetStatEnum(String value) {
		this.value = value;
	}
	...
	public static TransferNetStatEnum getApproveStatEnumByOrdinal(int value) {
		for (TransferNetStatEnum type : TransferNetStatEnum.values()) {
	...
	public static TransferNetStatEnum getApproveStatEnumByValue(String value) {
		for (TransferNetStatEnum type : TransferNetStatEnum.values()) {
